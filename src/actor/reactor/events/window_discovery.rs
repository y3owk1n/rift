...
impl WindowDiscoveryHandler {
                ...
                reactor
                    ...
                    .filter_map(|(&wid, state)| {
                        ...
                        let is_on_active_space =
                            active_space_windows.as_ref().is_some_and(|set| set.contains(&ws_id));
    ...
    fn process_window_list(
        ...
        let any_recent = new.iter().any(|(_, info)| {
            info.sys_id
                .is_some_and(|wsid| reactor.app_manager.is_wsid_recent(wsid, APP_RULE_TTL_MS))
        });
    ...
    pub fn emit_layout_events(
        ...
        tracing::debug!(
            ?pid,
            count = reactor.window_manager.window_ids.len(),
            "Third loop: checking window_ids"
        );
        for (&_wsid, &wid) in &reactor.window_manager.window_ids {
            if wid.pid != pid {
                continue;
        ...
        let spaces_to_process: Vec<SpaceId> = if active_spaces.is_empty() {
            tracing::debug!(
                "No active spaces detected, using all known spaces for discovered windows"
            );
            screens
                .iter()
